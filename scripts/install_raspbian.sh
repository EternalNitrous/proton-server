...
POWER_CONTROL_MARKER="/etc/proton-server/power-control-enabled"
POWER_CONTROL_SUDOERS="/etc/sudoers.d/proton-server-power"
...
write_power_control_config() {
    run $SUDO mkdir -p /etc/proton-server /etc/sudoers.d
    run $SUDO touch "$POWER_CONTROL_MARKER"

    {
        echo "$SERVICE_USER ALL=(root) NOPASSWD: /usr/sbin/shutdown -h now, /usr/sbin/reboot"
    } | $SUDO tee "$POWER_CONTROL_SUDOERS" >/dev/null
    run $SUDO chmod 440 "$POWER_CONTROL_SUDOERS"

    if command -v visudo >/dev/null 2>&1; then
        run $SUDO visudo -cf "$POWER_CONTROL_SUDOERS"
    fi
}
